    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-6 py-16">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-semibold text-[28px] tracking-tight text-foreground">Buat akun GoLearn</h1>
          <p className="text-[#86868B] text-[15px] mt-2">Gratis selamanya. Mulai belajar Go hari ini.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[13px] font-medium text-foreground mb-1.5">Nama lengkap</label>
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPw ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Min. 8 karakter"
              ...
              <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#86868B]">
        ...
        <p className="text-center text-[13px] text-[#86868B] mt-6">
          Sudah punya akun?{" "}
          <Link href="/login" className="text-[#0071E3] font-medium hover:underline">Masuk</Link>
        </p>
      </div>
    </div>
  );
